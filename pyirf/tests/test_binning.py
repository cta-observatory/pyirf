...
def test_join_bin_hi_lo():
    """Test join_bin_hi_lo function"""
    bins = np.array([np.logspace(-1,3, 20)*u.TeV])
    bins_lo = bins[:,:-1]
    bins_hi = bins[:,1:]

    bins_joint = binning.join_bin_hi_lo(bins_lo,bins_hi)
    assert np.allclose(bins_joint, bins, rtol=1.e-5)


def test_split_bin_hi_lo():
    """Test split_bin_hi_lo function"""
    bins = np.array([np.logspace(-1,3, 20)*u.TeV])
    bins_lo_true = bins[:,:-1]
    bins_hi_true = bins[:,1:]

    bin_lo, bin_hi = binning.split_bin_hi_lo(bins)
